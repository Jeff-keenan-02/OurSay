...
-- cached highest non-expired verification level
CREATE TABLE users (
...
CREATE TABLE verifications (
  ...
  -- region certification (ONLY for residence)
  region_code TEXT,
  ...
  CONSTRAINT residence_requires_region
    CHECK (
      (type = 'residence' AND region_code IS NOT NULL)
      OR
      (type != 'residence' AND region_code IS NULL)
    ),
...
CREATE TABLE petitions (
  ...
  required_verification_tier SMALLINT NOT NULL
    CHECK (required_verification_tier BETWEEN 2 AND 3),
...
-- fine to be user-linked as no opinions 
CREATE TABLE poll_group_progress (
  user_id INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  poll_group_id INTEGER NOT NULL REFERENCES poll_groups(id) ON DELETE CASCADE,

  -- 0 = not started, 1 = in progress, 2 = completed
  status SMALLINT NOT NULL CHECK (status IN (0,1,2)),
  completed_polls INT NOT NULL DEFAULT 0,

  updated_at TIMESTAMP DEFAULT NOW(),
  PRIMARY KEY (user_id, poll_group_id)
);

CREATE OR REPLACE FUNCTION update_poll_group_progress_timestamp()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE TRIGGER poll_group_progress_updated_at_trigger
BEFORE UPDATE ON poll_group_progress
FOR EACH ROW
EXECUTE FUNCTION update_poll_group_progress_timestamp();
...
VALUES
(1, 'Should vacant properties be taxed?',
 'Many properties sit empty while rents rise. Should there be penalties?',
 2),
(2, 'Free public transport — good idea?',
 'Would this reduce congestion or strain the system?',
 1),
(4, 'Is Ireland doing enough on climate change?',
 'Targets exist, but are we actually meeting them?',
 3),
